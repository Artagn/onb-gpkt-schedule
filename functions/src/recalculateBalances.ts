 ...
 */
const isWorkEligibleForBalance = (scheduleData: any, holidays: string[], workPeriods: any[]): boolean => {
 ...
 */
export const recalculateAllBalances = functions.https.onCall(async (data, context) => {
    ...
    const compLeavesUsed: Record<string, number> = {};
    ...
    const approvedLeaveMap: Record<string, Set<string>> = {};
    leavesSnap.docs.forEach((doc) => {
        const leave = doc.data();
    ...
    const earnedBalance: Record<string, number> = {};

    for (const sDoc of schedulesSnap.docs) {
        const sched = sDoc.data();

        // Skip Nghỉ bù items
        if (sched.jobId === "JOB_NGHI_BU") continue;
