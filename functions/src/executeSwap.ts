 ...
 */
export const approveSwapRequest = functions
    .region("asia-southeast1")
        ...
        try {
            const result = await db.runTransaction(async (transaction) => {
                ...
                if (!isManager && callerEmail) {
                    ...
                    callerEmpId = callerData?.employeeId || "";
                ...
                const swapItems = swapItemsSnap.docs.map(d => ({ ...d.data(), id: d.id, ref: d.ref } as any));
                ...
                if (request.requestShift === 'Tối') {
                    ...
                    const nextItems = nextItemsSnap.docs.map(d => ({ ...d.data(), id: d.id, ref: d.ref } as any));
                ...
                const newIdsA = itemA.employeeIds.filter((id: string) => id !== request.requesterId);
                ...
                if (itemB) {
                    const newIdsB = itemB.employeeIds.filter((id: string) => id !== request.targetId);
                ...
                if (request.requestShift === 'Tối') {
                    if (restItemA) {
                        const newRestIds = restItemA.employeeIds.filter((id: string) => id !== request.requesterId);
                    ...
                    for (const item of itemsB_Next) {
                        const newJobIds = item.employeeIds.filter((id: string) => id !== request.targetId);
