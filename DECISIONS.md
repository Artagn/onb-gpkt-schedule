...
## 41. Lịch cố định của job "Chia hàng ngày" không tính là "bận" & Hiển thị lịch theo buổi (v4.5.2)
- **Vấn đề:** Sau ADR #40, bảng Phân công hàng ngày vẫn sai màu và thiếu thông tin. Ví dụ Trần Tường Duy ngày 25/09/2026: Sáng có lịch cố định "Chuyển đổi & 1-1" (job_27), Chiều "TƯ VẤN LITE".
    1. `busyMap` đánh dấu mọi bản ghi `schedule` là "bận", kể cả job_27 — vốn chính là công việc được chia tại màn hình này. Duy bị coi là bận cả ngày nên hàng tô trắng, trong khi đúng ra phải là màu xanh dương (nhận việc buổi Sáng). Các nhân viên chỉ có lịch job_27 (Hồng Chung, Khánh Vân, Anh Thư, Hải Hà, Linh Phương) cũng sai màu tương tự.
    2. `isScheduledForJob` chỉ so theo ngày, không theo buổi: lọc buổi Chiều vẫn mở ô nhập job_27 cho Duy dù lịch job này của Duy chỉ có buổi Sáng.
    3. Cột nhân viên chỉ hiện email, Coordinator không biết từng người đang làm gì trong mỗi buổi để hiểu vì sao hàng có màu đó.
- **Quyết định:**
    - `busyMap` bỏ qua các bản ghi `schedule` có `jobId` thuộc nhóm "Chia hàng ngày" (`dailyJobIdSet`, tính trên toàn bộ `jobs` kể cả job đã tắt để dữ liệu cũ vẫn đúng). Nghỉ phép (`leaves`) và lịch của job nhóm khác vẫn tính là bận như cũ.
    - `isScheduledForJob` lọc thêm theo `shift` khi bộ lọc buổi là `Sáng`/`Chiều`; chế độ "Cả ngày" giữ nguyên (tính cả 2 buổi).
    - Thêm `shiftInfoMap` + `getEmployeeShiftInfo` để hiển thị dưới tên nhân viên: `Sáng: <job>` / `Chiều: <job>`, hoặc "Rảnh" / "Nghỉ" / "Nghỉ (chờ duyệt)". Chỉ hiện ở chế độ xem 1 ngày.
- **Lý do:** Ý nghĩa của "bận" ở màn hình này là "không thể nhận việc chia hàng ngày", nên lịch của chính job chia hàng ngày phải được coi là thời gian làm việc chia hàng chứ không phải thời gian bận. Loại trừ theo `group` thay vì theo `DEFAULT_JOB_IDS` để mọi job "Chia hàng ngày" trong tương lai tự động đúng mà không phải cập nhật danh sách. Thay đổi này cũng làm mất tác dụng của bẫy "AND triệt tiêu" mô tả ở ADR #40, nhưng vẫn giữ tách tiêu chí trong `isSuitableForJob` vì job mặc định còn phải yêu cầu có lịch cố định.
